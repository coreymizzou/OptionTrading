...
def get_sentiment(ticker):
    ...
    try:
        ...
        if not data:
            return 0
        score_sum = sum(float(item.get("overall_sentiment_score", 0)) for item in data)
        return round(score_sum / len(data), 3)
    except Exception:
        return 0
...
def choose_strategy(technical, iv_rank, sentiment):
    if iv_rank is None:
        return None

    bullish_bias = (technical == 'Bullish') or (sentiment and sentiment >= 2)
    bearish_bias = (technical == 'Bearish') or (sentiment and sentiment <= -2)

    # Strong IV edge — sell premium
    if iv_rank >= 70:
        if bullish_bias:
    ...
    # Moderate IV — blend of credit/debit
    elif 50 <= iv_rank < 70:
        if bullish_bias:
    ...
    # Low IV — long premium only
        ...
        if bullish_bias:
            ...
            return None  # Avoid neutral low-IV setups

    else:
        return None

    # Optional: enforce per-strategy limit
...
def evaluate_strategies(ticker):
    ...
    raw_sentiment = get_sentiment(ticker)
    sentiment = (raw_sentiment + 1) * 2.5  # Normalizes -1 to 1 → 0 to 5
    ...
    strategy = choose_strategy(technical, iv_rank, sentiment)
    if not strategy:
        return
    ...
    if strategy == "Put Credit Spread":
        ...
        for i in range(len(puts) - 1):
            ...
            if short['bid'] and long['ask'] and short['strike'] < long['strike']:
                spread_credit = short['bid'] - long['ask']
                spread_risk = long['strike'] - short['strike'] - spread_credit

                if 0 < spread_credit < spread_risk:
                    if spread_credit > max_credit:
                        max_credit = spread_credit
                        best_spread = (short, long)

        if not best_spread:
            print(f"[SKIP] No favorable {strategy} found for {ticker}")
            return

        short, long = best_spread
        credit = max_credit
        ...
        for i in range(len(puts) - 1):
            ...
            if short['bid'] and long['ask'] and short['strike'] < long['strike']:
                ...
                if 0 < spread_credit < spread_risk:  # Ensure positive credit and proper risk profile
                    if spread_credit > max_credit:
                        max_credit = spread_credit
                        best_spread = (short, long)
        ...
        short, long = best_spread  # <== Add this
        credit = max_credit
        risk = abs(long['strike'] - short['strike']) - credit
    ...
    if strategy in ["Iron Condor", "Iron Butterfly"]:
        ...
        selected = next(
            (o for o in opt_list if o.get("bid", 0) >= 0.25 and o.get("ask") is not None and o.get("strike") is not None),
            None
        )

        if selected is None:
            print(f"[SKIP] No viable option found for {ticker} using {strategy}")
            return

        try:
            if strategy == "Cash-Secured Put":
                credit = selected["bid"]
                risk = selected["strike"] - credit
            else:
               cost = selected["ask"]
               if cost > 2.5:  # skip overly expensive options
                    print(f"[SKIP] Option cost too high (${cost}) for {ticker} using {strategy}")
                    return
               risk = cost  # You can lose what you pay for the call/put
               credit = -cost  # To maintain compatibility with rest of code

        except Exception as e:
            print(f"[ERROR] Failed processing {ticker} with {strategy}: {e}")
            return

        # Always update strike display
        trade_details["Strikes"] = (
            f"  Long Call Strike: {selected['strike']}" if strategy == "Long Call" else
            f"  Long Put Strike: {selected['strike']}" if strategy == "Long Put" else
            f"  Put Strike: {selected['strike']}"
        )
