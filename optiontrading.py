...
def get_iv_from_tradier_chain(options_chain):
    ivs = [opt.get("implied_volatility") for opt in options_chain if opt.get("implied_volatility") is not None]
    if not ivs:
        return None
    return round(np.mean(ivs) * 100, 2)
...
def evaluate_strategies(ticker):
    ...
    if strategy == "Put Credit Spread":
        ...
        sp, lp = get_spread_legs(puts)
        sc, lc = get_spread_legs(calls)
        if not all([sp, lp, sc, lc]): return
        credit = (sp['bid'] - lp['ask']) + (sc['bid'] - lc['ask'])
        risk = max(abs(lp['strike'] - sp['strike']), abs(lc['strike'] - sc['strike'])) - credit
        trade_details.update({"Short Put Strike": sp['strike'], "Long Put Strike": lp['strike'], "Short Call Strike": sc['strike'], "Long Call Strike": lc['strike']})

    elif strategy == "Iron Butterfly":
        strikes = [o['strike'] for o in puts]
        if not strikes: return
        mid_strike = strikes[len(strikes)//2]
        if mid_strike not in available_strikes: return
        sp = next((p for p in puts if p['strike'] == mid_strike), None)
        sc = next((c for c in calls if c['strike'] == mid_strike), None)
        lp = next((p for p in puts if p['strike'] == mid_strike - 5), None)
        lc = next((c for c in calls if c['strike'] == mid_strike + 5), None)
        if not all([sp, sc, lp, lc]): return
